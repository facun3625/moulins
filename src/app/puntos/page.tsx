          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex flex-col gap-2">
              {pendingRedemptions.map((r) => (
                <div
                  key={r.id}
                  className="flex items-center justify-between gap-2 rounded-xl border border-primary bg-muted px-4 py-3 text-sm"
                >
                  <span className="font-medium">{r.coupon.code}</span>
                  <span className="text-muted-foreground">
                    {formatDiscount(r.coupon.discountType, Number(r.coupon.discountValue))} — usalo en el checkout
                  </span>
                </div>
              ))}
            </div>
