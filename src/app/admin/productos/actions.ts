...
export async function deleteStockGroup(id: string) {
  await requireAdmin();
  const variantCount = await prisma.productVariant.count({ where: { stockGroupId: id } });
  if (variantCount > 0) {
...
// nuevas de un producto que se creó "simple") queda individual.
async function ensureDedicatedStockGroup(tx: Pick<PrismaClient, "stockGroup">, fallbackName: string) {
...
async function resolveVariantStockGroupId(
  ...
  if (rawValue === "__siblings__") {
    ...
    if (!groupId) {
      groupId = await ensureDedicatedStockGroup(tx, dedicatedName);
      siblingGroupCache.set("__siblings__", groupId);
    }
    return groupId;
  }
  if (rawValue === "__solo__") {
    return ensureDedicatedStockGroup(tx, dedicatedName);
  }
  const group = await tx.stockGroup.findUnique({ where: { id: rawValue } });
  ...
  const product = await prisma.$transaction(async (tx) => {
    ...
    for (let i = 0; i < parsed.variants.length; i++) {
      ...
      const dedicatedName = [parsed.name, [v.gusto, v.tamano].filter(Boolean).join(" · ")]
        .filter(Boolean)
        .join(" — ");
      const stockGroupId = await resolveVariantStockGroupId(tx, v.stockGroupId, siblingGroupCache, dedicatedName);
      variantsData.push({
        gusto: v.gusto || null,
...
export async function duplicateProduct(id: string) {
  ...
  const copy = await prisma.$transaction(async (tx) => {
    ...
    for (const v of source.variants) {
      ...
      if (!newGroupId) {
        newGroupId = await ensureDedicatedStockGroup(tx, copyName);
